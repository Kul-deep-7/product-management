...
const DisplayProducts = () => {
    ...
    const [form, setForm] = useState({
        ...
        mrp: "", sellingPrice: "", brandName: "", exchangeEligibility: "",
    });
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            setShowModal(false);
            await fetchProducts();
            setForm({ productName: "", productType: "", quantityStock: "", mrp: "", sellingPrice: "", brandName: "", exchangeEligibility: "" });
            setImageFile([]);
            setImagePreview([]);
                    ...
                    <div className="bg-white rounded-xl w-full max-w-md mx-4 p-6 relative max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="flex flex-col gap-4">
                            <div>
                                ...
                                <input name="productName" value={form.productName} onChange={handleChange} placeholder="Enter your product Name" className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm outline-none focus:border-blue-500" />
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-gray-700 mb-1">Product Type</label>
                                <select name="productType" value={form.productType} onChange={handleChange} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm outline-none focus:border-blue-500 text-gray-500">
                            ...
                            <div>
                                ...
                                <input name="quantityStock" value={form.quantityStock} onChange={handleChange} placeholder="Total numbers of Stock available" type="number" className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm outline-none focus:border-blue-500" />
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-gray-700 mb-1">MRP</label>
                                <input name="mrp" value={form.mrp} onChange={handleChange} placeholder="Price" type="number" className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm outline-none focus:border-blue-500" />
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-gray-700 mb-1">Selling Price</label>
                                <input name="sellingPrice" value={form.sellingPrice} onChange={handleChange} placeholder="Selling Price" type="number" className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm outline-none focus:border-blue-500" />
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-gray-700 mb-1">Brand Name</label>
                                <input name="brandName" value={form.brandName} onChange={handleChange} placeholder="Brand Name" className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm outline-none focus:border-blue-500" />
                            </div>
                            <div>
                                <div className="flex items-center justify-between mb-1">
                                    <label className="text-xs font-medium text-gray-700">Upload Product Images</label>
                                    {imagePreview.length > 0 && (
                                        <span onClick={() => document.getElementById("imageInput").click()} className="text-xs text-blue-600 font-medium cursor-pointer">
                                            Add More Photos
                                        </span>
                                    )}
                                </div>
                                <div className="flex gap-2 flex-wrap">
                                    {imagePreview.map((src, index) => (
                                        <div key={index} className="relative">
                                            ...
                                            <button onClick={() => handleRemoveImage(index)} className="absolute -top-1 -right-1 bg-white rounded-full text-gray-500 text-xs w-4 h-4 flex items-center justify-center border border-gray-300">✕</button>
                                        </div>
                                    ))}
                                    {imagePreview.length === 0 && (
                                        <div onClick={() => document.getElementById("imageInput").click()} className="border border-dashed border-gray-300 rounded-md px-3 py-4 text-center cursor-pointer hover:border-blue-400 w-full">
                                            <p className="text-xs text-gray-400">Click to browse</p>
                                            <p className="text-xs text-blue-600 font-medium">Browse</p>
                                        </div>
                                    )}
                                </div>
                                <input id="imageInput" type="file" accept="image/*" multiple className="hidden" onChange={handleImageChange} />
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-gray-700 mb-1">Exchange or return eligibility</label>
                                <select name="exchangeEligibility" value={form.exchangeEligibility} onChange={handleChange} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm outline-none focus:border-blue-500 text-gray-500">
                            ...
                            <div className="flex justify-end mt-2">
                                <button onClick={handleCreate} disabled={loading} className="px-6 py-2 rounded-md text-white text-sm font-semibold disabled:opacity-70" style={{ background: "#002283" }}>
